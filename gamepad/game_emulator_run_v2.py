...
ADDR = ("127.0.0.1", 55002)  # IP:port of the TeleopEnv listener
...
class AxisControlApp(tk.Tk):
    """Отправляет 62-элементный вектор с целевой частотой STREAM_RATE_HZ.
    ...
    def __init__(self):
        super().__init__()
        self.title("Axis Control (29 DOF + XY speed + yaw + flip)")
    ...
    def _build_ui(self):
        ...
        ttk.Checkbutton(
            ctrl, text="Flip: hands (1) / feet (0)", variable=self.flip_var,
    ...
    def _collect_live_vector(self):
        ...
        return np.concatenate([visible_targets, mask, extra, [int(self.flip_var.get())]]).astype("<f4")
    ...
    def _collect_pattern_vector(self, frame_index, *, sync_ui):
        ...
        visible_targets = np.where(mask >= 0.5, targets, 0.0)
        vec = np.concatenate([visible_targets, mask, extra, [int(self.flip_var.get())]]).astype("<f4")
        return vec
    ...
    def _log_stream_status(self, now):
        ...
        if self.pattern_active and self.pattern_started_at is not None:
            ...
            state = "idle"
        print(f"[UDP] send_rate={actual_hz:.1f} Hz  {state}  flip={self.flip_var.get()}")
        self._status_started_at = now
        self._status_packet_count = 0
